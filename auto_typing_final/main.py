...
def transform_file_content(source: str, import_config: ImportConfig, ignore_global_vars: bool = False) -> str:
...
def main() -> int:
    ...
    for path in find_all_source_files(args.files):
        with path.open(open_mode) as file:
            source = file.read()
            transformed_content = transform_file_content(source=source, import_config=import_config, ignore_global_vars=args.ignore_global_vars)
